...
begin
  ...
  begin
    ...
    Assign(Plyk, FileName);
    {$I-}
    Rewrite(Plyk);
    {$I+}
    if IOResult <> 0 then
    begin
      MessageBox('Blad podczas zapisu do pliku!', nil, mfError or mfOKButton);
    end
    else
    begin
      writeln(Plyk, '=== RAPORT ===');
...
begin
  ...
  ReportDlg^.Insert(New(PButton, Init(R, '~Z~apisz do pliku', 101, bfNormal)));
  
  R.Assign(40, 11, 55, 13);
  ReportDlg^.Insert(New(PButton, Init(R, '~Z~amknij', cmCancel, bfDefault)));
  
  case Desktop^.ExecView(ReportDlg) of
    101: begin
